...
const loadUser = function(req,res){
  ...
  if(sessionid && user){
    req.user = user;
    req.userAgent = this.users.createUser(user.Name);
  }
};
...
const serveStaticPage = function(req,res){
  ...
  servePage(this.fs,filePath,res);
}
...
const writeInDatabase = function(req,res){
  if (req.user) {
    let database = this.users.getUsers();
...
const getHome = function(req,res){
  ...
  let userData = this.users.getUserData(req.user.Name);
...
const serveTodo = function(req,res){
  ...
  let viewContent = this.fs.readFileSync('./public/view.html','utf8');
