class Html {
  ...
  createTextTagWithTitle(title,name,value){
    return `<b>${title}: </b>${this.createTextTag(name,value)}`;
  }

  getViewContent(){
    let todo = this.content;
    let title = this.createTextTagWithTitle('Title','title',todo.title);
    let description = this.createTextTagWithTitle('Description','description',todo.description)
    let itemIds = Object.keys(todo.items);
    let items=itemIds.map((id)=>{
      let status = todo.items[id].status ? "checked" : '';
      return `${this.createCheckBox('itemId',id,status)}
      ${this.createTextTag('items',`${todo.items[`${id}`].item}`)}`;
    }).join('');
    let submit = this.createButtonTag('todoId',todo.todoId,'save','/addtodo');
